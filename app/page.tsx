...
const nav = [
  ...
  { label: "Công cụ & API", icon: Wrench },
  { label: "Cài đặt AI", icon: Settings2 },
];
      ...
      <section className="app-main">
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            <div className="factory-status"><span /><strong>FPT AI Factory</strong><em>Đã kết nối</em></div>
  ...
  return <>
    ...
    <section className="agent-grid">
      {agents.map((agent, index) => <article className="agent-card card" key={agent.name}>
        ...
        <div className="provider"><span>AI</span><div><small>Nhà cung cấp mô hình</small><strong>FPT AI Factory</strong></div><CheckCircle2 size={16} /></div>
        <div className="agent-metrics"><div><BookOpen size={16} /><strong>{agent.docs}</strong><small>Tài liệu</small></div><div><Wrench size={16} /><strong>{agent.tools}</strong><small>Công cụ</small></div><div><Clock3 size={16} /><strong>{index === 0 ? "4,1 giây" : "3,8 giây"}</strong><small>Phản hồi TB</small></div></div>
        <div className="updated"><RefreshCw size={13} /> Cập nhật {agent.updated}</div>
        <div className="card-actions"><Button onClick={index === 0 ? onOpen : undefined}>Mở chuyên gia <ChevronRight size={16} /></Button><Button variant="secondary"><TestTube2 size={16} /> Chạy thử</Button></div>
      </article>)}
    </section>
  </>;
}
...
function DetailScreen({ tab, setTab, onBack, openTool }: { tab: DetailTab; setTab: (t: DetailTab) => void; onBack: () => void; openTool: () => void }) {
  return <>
    ...
    <div className="detail-hero">
      ...
      <div className="detail-actions"><label>Phiên bản <select><option>v2.4 — Hiện tại</option><option>v2.3</option></select></label><Button variant="secondary"><Settings2 size={16} /> Chỉnh sửa</Button><Button><Play size={16} /> Chạy thử</Button></div>
    </div>
    <div className="tabs">{(["overview", "knowledge", "tools", "playground"] as DetailTab[]).map((t, i) => <button className={tab === t ? "active" : ""} onClick={() => setTab(t)} key={t}>{["Tổng quan", "Kho tri thức", "Công cụ", "Khu vực thử nghiệm"][i]}{t === "knowledge" && <span>682</span>}</button>)}</div>
...
function OverviewTab() {
  return <div className="two-col detail-content">
    ...
    <section className="card model-card"><div className="card-heading"><div><h3>Cấu hình mô hình</h3><p>Kết nối và tham số suy luận</p></div><Badge tone="success">Đã kết nối</Badge></div>
      <div className="factory-panel"><span>AI</span><div><small>Nhà cung cấp</small><strong>FPT AI Factory</strong><em>Hạ tầng AI bảo mật tại Việt Nam</em></div><CheckCircle2 size={18} /></div>
      <label>Mô hình<select><option>FPT LLM Enterprise 70B</option><option>FPT LLM Pro 32B</option></select></label>
      <div className="slider-label"><span>Độ sáng tạo</span><strong>0,2</strong></div><input className="range" type="range" min="0" max="10" value="2" readOnly /><div className="range-notes"><span>Chính xác</span><span>Sáng tạo</span></div>
      <div className="field-grid"><label>Giới hạn ngữ cảnh<input value="32.000 Token" readOnly /></label><label>Thời gian chờ<input value="60 giây" readOnly /></label></div>
      <div className="connection-row"><div><span className="pulse" /><p><strong>Kết nối ổn định</strong><small>Kiểm tra lúc 14:02 hôm nay</small></p></div><strong>128 mili giây</strong></div>
      <Button variant="secondary" className="full"><Link2 size={16} /> Kiểm tra kết nối</Button>
    </section>
  </div>;
}

const documents = [
  ["Quy trình cấp tín dụng 2026.pdf", "Quy trình", "4,8 MB", "15/07/2026", "Đã lập chỉ mục"],
  ["Quy định tài sản bảo đảm.pdf", "Chính sách", "2,1 MB", "12/07/2026", "Đã lập chỉ mục"],
  ["Chính sách chấm điểm tín dụng.pdf", "Mô hình", "3,6 MB", "08/07/2026", "Đã lập chỉ mục"],
  ["Biểu phí tín dụng doanh nghiệp.pdf", "Biểu phí", "1,2 MB", "01/07/2026", "Đang xử lý"],
  ["Chính sách tín dụng 2024.pdf", "Chính sách", "5,4 MB", "22/12/2024", "Hết hiệu lực"],
];

function KnowledgeTab() {
  return <div className="detail-content"><div className="mini-stats"><div><BookOpen /><p><strong>682</strong><small>Tổng tài liệu</small></p></div><div><CheckCircle2 /><p><strong>658</strong><small>Đã lập chỉ mục</small></p></div><div><Clock3 /><p><strong>14</strong><small>Đang xử lý</small></p></div><div><AlertTriangle /><p><strong>10</strong><small>Lỗi hoặc hết hiệu lực</small></p></div></div>
    <section className="card table-card"><div className="table-toolbar"><div><h3>Tài liệu nghiệp vụ</h3><p>Nguồn tri thức được sử dụng khi phân tích hồ sơ</p></div><label className="search-box"><Search size={17} /><input placeholder="Tìm tài liệu..." /></label><Button variant="ghost"><Filter size={16} /> Danh mục</Button><Button variant="secondary"><Box size={16} /> Nhập thư mục</Button><Button><Upload size={16} /> Tải tài liệu lên</Button></div>
      <div className="data-table"><div className="tr th"><span>TÊN TÀI LIỆU</span><span>DANH MỤC</span><span>DUNG LƯỢNG</span><span>CẬP NHẬT</span><span>TRẠNG THÁI</span><span /></div>{documents.map((d) => <div className="tr" key={d[0]}><span className="doc-name"><FileText size={18} /><strong>{d[0]}</strong></span><span>{d[1]}</span><span>{d[2]}</span><span>{d[3]}</span><span><Badge tone={d[4] === "Đã lập chỉ mục" ? "success" : d[4] === "Đang xử lý" ? "info" : "warning"}>{d[4]}</Badge></span><button><MoreHorizontal size={17} /></button></div>)}</div>
    </section></div>;
}

const tools = ["Tra cứu CIC API", "Core Banking", "Công cụ tính DTI", "Định giá tài sản bảo đảm", "Tra cứu giao dịch khách hàng", "Xuất báo cáo tín dụng", "API hồ sơ khách hàng", "API yêu cầu tài liệu"];
function ToolsTab({ openTool }: { openTool: () => void }) {
  return <div className="detail-content"><div className="tool-summary"><div><h3>Công cụ đã kết nối</h3><p>Cho phép chuyên gia truy xuất và thực hiện nghiệp vụ có kiểm soát</p></div><strong>8 <span>/ 10 công cụ đã cấu hình</span></strong><Button><Plus size={16} /> Thêm công cụ</Button></div>
    <div className="tools-grid">{tools.map((tool, i) => <button className="tool-card card" key={tool} onClick={openTool}><span className={`tool-icon ${i % 4 === 0 ? "blue" : i % 4 === 1 ? "purple" : i % 4 === 2 ? "cyan" : "green"}`}><Database size={19} /></span><div><h4>{tool}</h4><p>{i < 4 ? "API" : "Công cụ nội bộ"} · Thử lúc {i % 2 ? "hôm qua" : "14:01"}</p><div><Badge tone="success">Đã kết nối</Badge>{i % 3 === 0 && <Badge tone="warning">Cần phê duyệt</Badge>}</div></div><ChevronRight size={18} /></button>)}</div>
  </div>;
}

function PlaygroundTab() {
  const [ran, setRan] = useState(false);
  return <div className="playground detail-content"><section className="card chat-panel"><div className="card-heading"><div><h3>Thử nghiệm chuyên gia</h3><p>Kiểm tra phản hồi trước khi đưa vào vận hành</p></div><Badge tone="success">FPT AI Factory</Badge></div><div className="suggestions"><span>Gợi ý kiểm thử</span><button>Đánh giá sơ bộ khoản vay</button><button>Kiểm tra điều kiện DTI</button></div><div className="chat-space">{ran ? <><div className="chat user">Đánh giá sơ bộ khả năng vay 2,5 tỷ của khách hàng này.</div><div className="chat ai"><strong><Sparkles size={15} /> Chuyên gia tín dụng</strong><p>Khách hàng có khả năng đáp ứng điều kiện vay ở mức trung bình–tốt. Điểm CIC 742 và DTI 38,5% nằm trong ngưỡng cho phép. Cần bổ sung tờ khai thuế gần nhất trước khi đưa ra quyết định.</p><span>Độ tin cậy 87% · 3,8 giây</span></div></> : <div className="empty-chat"><MessageSquareText /><strong>Bắt đầu một lượt kiểm thử</strong><p>Nhập tình huống nghiệp vụ để xem cách chuyên gia phân tích.</p></div>}</div><div className="chat-input"><textarea defaultValue="Đánh giá sơ bộ khả năng vay 2,5 tỷ của khách hàng này." /><Button onClick={() => setRan(true)}><Play size={16} /> Chạy thử</Button></div></section>
    <section className="card inspector"><div className="card-heading"><div><h3>Chi tiết thực thi</h3><p>Theo dõi từng bước xử lý</p></div><Activity size={19} /></div>{["Tiếp nhận yêu cầu", "Truy xuất 5 đoạn tài liệu", "Gọi Tra cứu CIC API", "Gọi công cụ tính DTI", "Tạo báo cáo đánh giá"].map((s, i) => <div className={`inspect-step ${ran ? "done" : i === 0 ? "active" : ""}`} key={s}><span>{ran ? <Check size={13} /> : i + 1}</span><div><strong>{s}</strong><small>{ran ? `${(0.3 + i * .4).toFixed(1).replace(".", ",")} giây` : i === 0 ? "Sẵn sàng" : "Đang chờ"}</small></div></div>)}<div className="inspect-meta"><div><span>Thời gian</span><strong>{ran ? "3,8 giây" : "—"}</strong></div><div><span>Token</span><strong>{ran ? "1.842" : "—"}</strong></div><div><span>Mô hình</span><strong>FPT LLM 70B</strong></div></div></section>
  </div>;
}
...
function TeamScreen({ selected, setSelected, run, onRun, onOpenRun }: { selected: string; setSelected: (s: string) => void; run: number; onRun: () => void; onOpenRun: () => void }) {
  return <><PageHeading title="Thiết lập đội chuyên gia" subtitle="Đội chuyên gia thẩm định hồ sơ vay doanh nghiệp"><Button variant="secondary"><Check size={16} /> Lưu đội chuyên gia</Button><Button onClick={onRun}><Play size={16} /> Chạy thử</Button></PageHeading>
...
function PropertiesPanel({ selected }: { selected: string }) {
  ...
  return <section className="card properties"><div className="panel-heading"><div><h3>Thuộc tính nút</h3><span>Cấu hình nhiệm vụ</span></div><button><PanelLeftClose size={17} /></button></div><div className="selected-agent"><span className="agent-icon blue"><CircleDollarSign size={18} /></span><div><small>CHUYÊN GIA ĐÃ CHỌN</small><strong>{name[selected]}</strong></div></div><label>Tên chuyên gia<input value={name[selected]} readOnly /></label><label>Nhiệm vụ được giao<textarea value="Phân tích năng lực tài chính, lịch sử tín dụng và đề xuất hạn mức phù hợp." readOnly /></label><label>Kết quả mong đợi<textarea value="Báo cáo đánh giá tín dụng có trích dẫn bằng chứng và mức độ tin cậy." readOnly /></label><label>Kho tri thức<select><option>Kho tín dụng doanh nghiệp</option></select></label><label>Công cụ được bật<div className="guardrails"><span><Check size={14} /> Tra cứu CIC API</span><span><Check size={14} /> Công cụ tính DTI</span><span><Check size={14} /> Core Banking</span></div></label><div className="field-grid"><label>Thời gian chờ<input value="60 giây" readOnly /></label><label>Số lần thử lại<input value="2 lần" readOnly /></label></div><label className="toggle-row"><span><strong>Yêu cầu phê duyệt</strong><small>Trước hành động bên ngoài</small></span><input type="checkbox" defaultChecked /></label></section>;
}
...
function ToolDrawer({ onClose }: { onClose: () => void }) {
  ...
  return <DrawerShell title="Cấu hình công cụ" subtitle="Tra cứu CIC API" onClose={onClose}><div className="drawer-tool"><span className="tool-icon blue"><Database /></span><div><h3>Tra cứu CIC API</h3><p>Truy xuất lịch sử tín dụng và dư nợ hiện tại của khách hàng từ hệ thống CIC.</p></div></div><div className="drawer-section"><h4>THÔNG TIN KẾT NỐI</h4><label>Điểm cuối API<input value="https://api.bank.local/v2/cic/lookup" readOnly /></label><label>Phương thức<div className="method"><strong>POST</strong><span>/customers/credit-history</span></div></label><label>Xác thực<select><option>Khóa API · Đã cấu hình</option></select></label></div><div className="drawer-section"><h4>CẤU TRÚC DỮ LIỆU</h4><label>Dữ liệu đầu vào<pre>{`{\n  "customer_id": "string",\n  "id_number": "string"\n}`}</pre></label><label>Dữ liệu đầu ra<pre>{`{\n  "credit_score": "number",\n  "debt_groups": "array"\n}`}</pre></label></div><label className="toggle-row"><span><strong>Yêu cầu phê duyệt</strong><small>Chuyên viên phải duyệt trước khi gọi công cụ</small></span><input type="checkbox" checked={approval} onChange={() => setApproval(!approval)} /></label>{tested && <div className="test-success"><CheckCircle2 /> Kết nối thành công · 142 mili giây</div>}<div className="drawer-actions"><Button variant="secondary" onClick={onClose}>Hủy</Button><Button onClick={() => setTested(true)}><Link2 size={16} /> Kiểm tra kết nối</Button></div></DrawerShell>;
}

function TraceDrawer({ onClose }: { onClose: () => void }) {
  const [raw, setRaw] = useState(false);
  return <DrawerShell title="Chi tiết thực thi" subtitle="14:03 · Lệnh gọi công cụ" onClose={onClose}><div className="trace-summary"><span className="agent-icon blue"><CircleDollarSign /></span><div><span>CHUYÊN GIA AI</span><strong>Chuyên gia tín dụng</strong><small>Đánh giá lịch sử tín dụng khách hàng</small></div><Badge tone="success">Hoàn thành</Badge></div><div className="trace-grid"><div><span>Loại hành động</span><strong>Lệnh gọi công cụ</strong></div><div><span>Thời gian</span><strong>1,2 giây</strong></div><div><span>Mô hình</span><strong>FPT LLM 70B</strong></div><div><span>Token</span><strong>486</strong></div></div><div className="drawer-section"><h4>CÔNG CỤ ĐÃ GỌI</h4><div className="evidence-head"><Database /><div><strong>Tra cứu CIC API</strong><small>HTTP 200 · 824 mili giây</small></div><Badge tone="success">Thành công</Badge></div><label>Tham số đầu vào<pre>{`{\n  "customer_id": "KH-0018293",\n  "purpose": "loan_assessment"\n}`}</pre></label><label>Kết quả<pre>{`{\n  "credit_score": 742,\n  "debt_group": 1,\n  "overdue_debt": 0\n}`}</pre></label></div><div className="drawer-section"><h4>BẰNG CHỨNG RAG LIÊN QUAN</h4><div className="evidence"><div><FileText /><p><strong>Chính sách chấm điểm tín dụng.pdf</strong><small>Trang 18 · Độ liên quan 94%</small></p></div><blockquote>“Khách hàng có điểm tín dụng từ 720 và không phát sinh nợ nhóm 3–5 trong 24 tháng gần nhất được xếp nhóm rủi ro thấp đến trung bình.”</blockquote></div></div>{raw && <pre className="raw-response">{`{"request_id":"req_82f1","status":"success","model":"fpt-llm-enterprise-70b"}`}</pre>}<Button variant="secondary" className="full" onClick={() => setRaw(!raw)}><Box size={16} /> {raw ? "Ẩn dữ liệu phản hồi gốc" : "Xem dữ liệu phản hồi gốc"}</Button></DrawerShell>;
}
